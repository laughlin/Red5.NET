...
public class RTMPProtocolEncoder extends BaseProtocolEncoder 
    ...
    public void doEncodeSharedObject(ISharedObjectMessage so, RTMP rtmp, IoBuffer out) {
        ...
        for (ISharedObjectEvent event : so.getEvents()) {
            ...
            if (eventType == Type.SERVER_CONNECT || eventType == Type.CLIENT_INITIAL_DATA || eventType == Type.CLIENT_CLEAR_DATA){
                out.put(type);
                out.putInt(0);
            } else if (eventType == Type.SERVER_DELETE_ATTRIBUTE || eventType == Type.CLIENT_DELETE_DATA || eventType == Type.CLIENT_UPDATE_ATTRIBUTE) {
                out.put(type);
                mark = out.position();
                ...
                out.putInt(mark, len);
            } else if (eventType == Type.SERVER_SET_ATTRIBUTE || eventType == Type.CLIENT_UPDATE_DATA){
            ...
            } else if (eventType == Type.CLIENT_SEND_MESSAGE || eventType == Type.SERVER_SEND_MESSAGE){
            ...
            } else if (eventType == Type.CLIENT_STATUS){
                out.put(type);
